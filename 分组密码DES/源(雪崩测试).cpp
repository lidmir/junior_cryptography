...
//加密
void Encrypt(int key[8],int txt[8],int out[8])
{
	cout<<"-------------分组密码加密-----------"<<endl;
	cout<<"密钥为：";
	for(int i=0;i<8;i++)
...
//统计密文改变位数的函数
int statistics(int out[8],int out1[8])
{
	int str[64],str1[64];
	for(int i=0;i<8;i++)
	{
...
void main()
{
	...
	//分组密码的加解密
	int out[8];
	cout<<"输入要进行加解密的明密文组的数字编码（0-9）：";
	int i;
	cin>>i;
	Encrypt(cases[i].key,cases[i].txt,out);
	Decrypt(cases1[i].key,cases1[i].txt);
	
	/*
	//雪崩效应
	cout<<"***********************************************"<<endl;
	cout<<"********雪崩效应检验：改变明文中的一位*********"<<endl;
	cout<<"***********************************************"<<endl;
	int out[8];
	int out1[8];
	int k=0;
	float avg=0;
	Encrypt(cases[0].key,cases[0].txt,out);
	for(int j=0;j<8;j++)
	{
		cases[0].txt[j]=0x02;
		Encrypt(cases[0].key,cases[0].txt,out1);
		cases[0].txt[j]=0x00;
		k=statistics(out,out1);
		cout<<"改变明文第"<<8*j+7<<"位，密文改变位数："<<k<<endl;
		avg+=k;
	}
	avg=avg/8;
	cout<<"***********************************************"<<endl;
	cout<<"********雪崩效应检验：改变密钥中的一位*********"<<endl;
	cout<<"***********************************************"<<endl;
	
	float avg1=0;
	Encrypt(cases[1].key,cases[1].txt,out);
	for(int j=0;j<8;j++)
	{
		cases[1].key[j]=0x03;
		Encrypt(cases[1].key,cases[1].txt,out1);
		cases[1].key[j]=0x01;
		k=statistics(out,out1);
		cout<<"改变密钥第"<<8*j+7<<"位，密文改变位数："<<k<<endl;
		avg1+=k;
	}
	avg1=avg1/8;
	cout<<endl<<"*******************************"<<endl;
	cout<<"************统计结果***********"<<endl;
	cout<<"*******************************"<<endl;
	cout<<"改变明文中的一位，密文平均改变位数："<<avg<<endl;
	cout<<"改变密钥中的一位，密文平均改变位数："<<avg1<<endl;
	*/
}
